...
- ✅ `prompt-creator` done, in two halves. Its bundled checklist now carries the reviewer-scoping deviation in-file (the item [MODEL-NOTES.md](./MODEL-NOTES.md) § *Prompting adjustments* records as deferred — that line is stale and will be corrected on the next `update-for-model` run, which owns that file). Both of its `description` fields were rewritten to drop the workflow summary. **Q6's gate is now live:** that rule is plugin-local until P1's before/after evidence exists, and the evals to produce it went unrun (`claude plugin eval` is early-access-gated and reads a different format). Get a runnable arm first — see Phase 2 — then decide whether the no-workflow-summary rule goes catalogue-wide in [CONVENTIONS.md](./CONVENTIONS.md).
