...
- ✅ `linear-flow` (skills `ln-triage`, `ln-whats-next`, `ln-ship-loop`, `ln-project-lifecycle`, `ln-issue-lifecycle`) — drives Linear through its MCP tools from a user-supplied conventions file, shipping generic with no workspace values baked in; shipped at `0.3.0` (`0.2.0` gained the PR relation gate; `0.3.0` anchored its bare trigger phrases to Linear). First plugin here with a runtime dependency on an external MCP server.
...
Choosing a target Claude model has knock-on effects on the other plugins' skills. `prompt-creator` `0.2.0` was the first one acted on; the Opus 5.5 prompt audit of 2026-09-23 covered the rest.
...
- ✅ Target model: **Claude Opus 5.5** (re-targeted from Opus 5 on 2026-09-23) — see [MODEL-NOTES.md](./MODEL-NOTES.md). All 61 skills were reviewed against it with the bundled `/claude-api prompt-audit`; the [report and triage](./audits/prompt-audit-2026-09-23.md) applied six edits across three plugins and declined or flagged the rest with reasons.
- ⏳ Trigger-language calibration, the part the audit could not settle: `description` wording tuned to stop a former model from *under*-triggering can make a newer, more instruction-sensitive model *over*-trigger. The description trims the audit proposed (`offers`, and `prompt-creator`'s near-synonym phrases) wait for trigger evals, which the runner can now measure (Phase 2).
- ⏳ Source or date the volatile statistics the audit flagged in `marketing-skills` (percentages, multiples, and prices stated as current fact); content work on derived text, listed in the [report](./audits/prompt-audit-2026-09-23.md).
- ✅ `linear-flow`'s two open review findings, re-decided by measurement ([anthropic-updates PRD](./prds/anthropic-updates-2026-09.md), P7 and Q2). The four unanchored trigger phrases — "close these", "relabel these", "groom the backlog", "track this" — are now anchored to Linear (`0.3.0`). Four negative eval cases put each bare phrase in a GitHub or local-file context: no `ln-*` skill fired in 12 of 12 runs before the anchoring, nor in 12 of 12 after. So `disable-model-invocation` stays off on all five skills, keeping auto-triggering. Still unmeasured: whether the anchored phrases trigger when Linear *is* meant — those positive cases sit in the legacy `evals.json` files.
- ✅ Decided: `update-for-model` stays scoped to living docs and never edits sibling plugins' `SKILL.md` files. Tuning skills for a model has a native tool now, `/claude-api prompt-audit`, which `skill-auditor` `0.2.0` runs.
