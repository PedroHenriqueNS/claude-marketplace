...
- ⏳ Target model chosen: **Claude Opus 5** — see [MODEL-NOTES.md](./MODEL-NOTES.md). Review the other 9 plugins' skills against its guidance. The likely hot spot is trigger-language calibration: `description` wording tuned to stop a former model from *under*-triggering can make a newer, more instruction-sensitive model *over*-trigger. Re-check the `disable-model-invocation` choices at the same time.
