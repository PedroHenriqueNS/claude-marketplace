...
1. **Ask which model to target — always, and first.** Fetch <https://platform.claude.com/docs/en/models/overview>, then offer the models that page actually lists (free text is fine too). **Never infer the target from the model running this session**, and never rely on a model list memorized from training — the lineup changes and stale lists are the failure this skill exists to prevent. If that fetch fails, do not fall back to a remembered lineup: ask the user to name the model and its API ID, tell them you could not verify it exists, and carry that warning into the report and into MODEL-NOTES.md.
